...
main = do
  args <- getArgs
  input <- fmap load $ readFile (args !! 0)
  let burst = if args !! 1 == "1" then burst1 else burst2
  let count = read (args !! 2)
  let result = execState (doN burst count) (initial input)
  putStrLn . show $ _infections result
